...
def display_password_list(window, location):
    ...
    for password in password_list:
        password = embed_pasword(password,size)
        display_line(window, password, location)

    # choose password
    return password_list[7]

def embed_pasword(password,size):
...
def get_guesses(window, password, location, attempts_left):
    ...
    line_x = 0
    hint_y = window.get_font_height()  - window.get_font_height() 
    ...
    while guess != password and attempts_left > 0:
        ...
        hint_y = display_hint(window,guess,hint_y,password)
        guess = prompt_user(window, prompt, location)
        attempts_left = attempts_left - 1
    return guess
...
def display_hint(window,guess,hint_y,password):
    window.draw_string(guess + ' INCORRECT', 300 ,hint_y)   
    window.update()
    ...
    correct = 0
    for letter in guess:  
        correct = check_letter(index,password,letter,correct)
        index = index + 1
        
    window.draw_string(str(correct)+'/7 IN MATCHING POSITIONS', 300 ,hint_y)  
    hint_y =  hint_y  + window.get_font_height()    
    return hint_y

def check_letter(index,password,letter,correct): 
